...
import com.hedera.services.yahcli.BaseCommandTest;
...
public class YahcliTest extends BaseCommandTest {
    ...
    public void testConstructor() {
        // Test the newly created instance from initializeCommand()
    ...
    @Test
    @DisplayName("CLI should parse schedule flag correctly with short option")
    public void testScheduleShortOptionParsing() {
        commandLine.parseArgs("-s");
        assertTrue(yahcli.isScheduled());
    }

    @Test
    @DisplayName("CLI should parse schedule flag correctly with long option")
    public void testScheduleLongOptionParsing() {
        commandLine.parseArgs("--schedule");
        assertTrue(yahcli.isScheduled());
    }
    ...
    public void testSubcommandRegistration() {
        ...
        String[] expectedSubcommands = {
                "help", "accounts", "keys", "sysfiles", "fees", "schedule",
                "freeze-abort", "freeze-only", "prepare-upgrade", "freeze-upgrade",
                "upgrade-telemetry", "version", "activate-staking", "nodes"
        };
    ...
    public void testValidOptionsWithInvalidValue() {
        ...
        assertTrue(exception.getMessage().contains("Invalid value for option"),
                "Error should be about the invalid value");
    }
    ...
    public void testComplexOptionCombination() {
        commandLine.parseArgs(
                ...
                "-p", "50",
                "-a", "0.0.3",
                "-f", String.valueOf(Long.MAX_VALUE),
                "-s",
                "-v", "DEBUG",
                "-c", "custom-config.yml"
        );
    ...
    public void testOptionsWithSubcommand() {
        ...
        assertEquals(0, exitCode);
        assertEquals("testnet", yahcli.getNet(),
                "Global option should be set even when subcommand is specified");
    }
    ...
    public void testDefaultConfigLocation() {
        ...
        assertEquals("config.yml", yahcli.getConfigLoc(),
                "Default config location should be 'config.yml'");
    }
    ...
    public void testOptionOverride() {
        ...
        assertEquals("mainnet", yahcli.getNet(),
                "Last specified option should override previous value");
    }
    ...
    public void testExtremelyLargeFixedFee() {
        ...
        assertEquals(Long.MAX_VALUE, yahcli.getFixedFee(),
                "CLI should handle maximum long value for fixed fee");
    }
